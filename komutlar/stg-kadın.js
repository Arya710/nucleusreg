...
if(!['814424825048334357'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) 
...
const tag = ''   
const kadınrol = message.guild.roles.cache.find(r => r.id === '814424836297588756') //kadın rol id
const kadınrol2 = message.guild.roles.cache.find(r => r.id === '814424836809293875')//kadın rol2 id
const kayıtsız = message.guild.roles.cache.find(r => r.id === '814424848205348894')//kayıtsız rol id
const genelchat = message.guild.channels.cache.find(c => c.id === '814838067067093032')//genel chat id
const savelog = message.guild.channels.cache.find(c => c.id === '814424915171999764')//savelog id
...
genelchat.send(`${member} Aramıza katıldı.`)
